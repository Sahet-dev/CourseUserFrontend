<template>
    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-r from-cyan-500 to-blue-500">
        <div class="w-full max-w-md bg-white bg-opacity-95 p-10 rounded-2xl shadow-2xl transform hover:scale-105 transition-transform duration-300">
            <h2 class="text-3xl font-extrabold mb-8 text-center text-gray-800 tracking-tight">{{ $t('login.title') }}</h2>

            <form @submit.prevent="login" class="space-y-6">
                <!-- Email Field -->
                <div class="relative">
                    <label for="email" class="absolute -top-2 left-2 inline-block bg-white px-1 text-xs font-medium text-gray-600">{{ $t('login.email.label') }}</label>
                    <input
                        type="email"
                        id="email"
                        v-model="form.email"
                ...
                <!-- Password Field -->
                <div class="relative">
                    <label for="password" class="absolute -top-2 left-2 inline-block bg-white px-1 text-xs font-medium text-gray-600">{{ $t('login.password.label') }}</label>
                    <input
                        type="password"
                        id="password"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        />
                        <label for="remember" class="ml-2 block text-sm text-gray-700">{{ $t('login.rememberMe') }}</label>
                    </div>
                    <a href="#" class="text-sm font-medium text-blue-600 hover:text-blue-500">{{ $t('login.forgotPassword') }}</a>
                </div>
                ...
                >
                    ...
                    {{ loading ? $t('login.button.loading') : $t('login.button.default') }}
                </button>
            </form>
