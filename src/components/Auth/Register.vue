<template>
    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-r from-cyan-500 to-blue-500 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8 bg-white bg-opacity-95 p-10 rounded-2xl shadow-2xl transform hover:scale-105 transition-transform duration-300">
            <div>
                <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900 tracking-tight">{{ $t('register.title') }}</h2>
                <p class="mt-2 text-center text-sm text-gray-600">
                    {{ $t('register.alreadyHaveAccount') }}
                    <a href="#" class="font-medium text-blue-600 hover:text-blue-500">{{ $t('register.signInLink') }}</a>
                </p>
            </div>
            <form class="mt-8 space-y-6" @submit.prevent="handleRegister">
                <div class="rounded-md shadow-sm -space-y-px">
                    ...
                    <div>
                        <label for="email-address" class="sr-only">{{ $t('register.form.email.label') }}</label>
                        <input
                            v-model="email"
                            id="email-address"
                            name="email"
                    ...
                    <div>
                        <label for="password_confirmation" class="sr-only">{{ $t('register.form.passwordConfirmation.label') }}</label>
                        <input
